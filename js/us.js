...
function renderProfiles(people) {
  ...
  const cards = people.map((p) => {
    ...
    return `
      <div class="profile-card">
        <div class="profile-avatar photo-editable" data-person-id="${p.id}">
          ${avatar}
          <span class="photo-edit-icon"><svg viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2Z"/><circle cx="12" cy="13" r="4"/></svg></span>
        </div>
        <h2>${p.name}</h2>
  ...
  // Click any avatar to pick a different photo from uploaded media.
  root.querySelectorAll('.profile-avatar').forEach((avatarEl) => {
    avatarEl.addEventListener('click', () => {
      const personId = avatarEl.dataset.personId;
      openMediaPicker(async ({ id, url }) => {
        avatarEl.innerHTML = `<img src="${url}" alt=""><span class="photo-edit-icon"><svg viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2Z"/><circle cx="12" cy="13" r="4"/></svg></span>`;
        if (db && !personId.startsWith('demo-')) {
          try {
            const { error } = await db.from('us').update({ photo_id: id }).eq('id', personId);
